...
{
    cmd_loadidentity();
    cmd_translate(F16(PLAYER1_SIZE / 2),F16(PLAYER1_SIZE / 2));
    cmd_rotate(a);
    cmd_translate(F16(-PLAYER1_WIDTH / 2),F16(-PLAYER1_HEIGHT / 2));
    cmd_setmatrix();
}
...
{
    cmd_loadidentity();
    cmd_translate(F16(HEART_SZ / 2),F16(HEART_SZ / 2));
    cmd_rotate(a);
    cmd_translate(F16(-HEART_WIDTH / 2),F16(-HEART_HEIGHT / 2));
    cmd_setmatrix();
}

#define BLOBS 32
typedef struct TrailClass {
    int idx;
    int x[BLOBS];
    int y[BLOBS];
    int sz[BLOBS];
...
static void polar2(uint16_t th, int r)
{
  int x, y;
  polar(x, y, r, th);
  Vertex2f(16 * 240 + x, 16 * 136 + y);
}
...
static void ray(uint16_t th, int r0, int r1)
{
  polar2(th, r0);
  polar2(th, r1);
  polar2(th + RAYSIZE / 2, r1);
  polar2(th + RAYSIZE / 2, r0);
  polar2(th, r0);
}
...
{
  Send_CMD(CLEAR(0, 1, 2)); //RJA Clear(0,1,0);
  ColorMask(0,0,0,0);
...
static void draw(int t)
{
  ...
  BitmapHandle(CLOUD_HANDLE);
  Cell(0);
  for (int i = 0; i < 20; i++) {
    byte lum = 128 + 5 * i;
    ColorA(lum);
    ColorRGB(lum, lum, lum);
    Vertex2f(state.clouds[i].x, state.clouds[i].y);
  ...
  state.p[0].x = 16 * 240 + rsin(16 * 120, a) + rsin(16 * 120, a2);
  state.p[0].y = 16 * 136 + rsin(16 * 70, a3);

  state.p[1].x = 16 * 240 + rsin(16 * 240, a);
  state.p[1].y = 16 * 100 + rsin(16 * 36, a2);

  state.p[2].x = 16 * 240 + rsin(16 * 240, a2);
  state.p[2].y = 16 * 135 + rsin(16 * 10, a) + rsin(16 * 18, a3);
  ...
  ColorA(0xff);
  BlendFunc(ONE, ONE_MINUS_SRC_ALPHA);
  BitmapHandle(PLAYER1_HANDLE);
  Begin(BITMAPS);
  for (int i = 0; i < 3; i++) {
    rotate_player(a + i * 0x7000);
    Cell(i);
    Vertex2f(state.p[i].x - (16 * PLAYER1_SIZE / 2),
                state.p[i].y - (16 * PLAYER1_SIZE / 2));
  }

  if (t > 480) {
    BlendFunc(SRC_ALPHA, ONE_MINUS_SRC_ALPHA);
    ColorA(min((t - 480) * 4, 255));
    Begin(BITMAPS);
    BitmapHandle(HEART_HANDLE);
    Cell(0);

    for (int i = 0; i < 20; i++) {
      if ((i & 3) == 0)
        rotate_heart(a + (i << 12));
      Vertex2f(state.hearts[i].x - (16 * HEART_SZ / 2), state.hearts[i].y);
  ...
  // RestoreContext(); cmd_number(0, 0, 26, 6, micros() - t0);
...
void setup()
{
  ...
  // wr32(REG_PWM_HZ, 18000); wr(REG_PWM_DUTY, 64);
  ...
  for (int i = 0; i < 20; i++) {
    state.hearts[i].x = random1(16 * 480);
    state.hearts[i].y = 16 * hy[i];
    state.clouds[i].x = 16 * (random1(480) - (CLOUD_WIDTH / 2));
